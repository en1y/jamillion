...
- **The countdown reads the server's clock, it does not own one, and it never reads the device's.** The served question carries `seconds_left` -- a duration, not an instant -- and the ring ticks four times a second off that budget minus `performance.now()`, submitting whatever is typed when it reaches zero; the backend's three-second grace absorbs the round trip. `deadline` is still served as the server's own truth. Reading the wall clock was the bug: a device whose time ran fast by more than the limit opened every question already expired. Nothing about the timer is persisted, so a reload just re-reads `seconds_left` from `POST /api/attempts`.
