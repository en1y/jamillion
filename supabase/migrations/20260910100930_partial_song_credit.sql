-- v0.9.1: getting part of a song or album question right pays what that part is
-- worth. The three fields reach the tower as one string joined with " — ", so a
-- wrong album sank the artist and the title with it: "Eminem — Rap God — The
-- Marshall Mathers LP2 (Deluxe)" matched no row and scored 0, though the key
-- holds "Eminem — Rap God" at 30.
--
-- The key already enumerates every combination, so the fallback is to try the
-- combinations of what the player actually typed and take the best-paying one.
-- Three fields is seven subsets; the bitmask below is that, in order.
...
BEGIN
    ...
    IF v_norm <> '' THEN
        ...
        -- Nothing matched whole. On a song or an album question, the fields the
        -- player did get right still count: score the best combination of them.
        ...
        IF NOT coalesce(ans.is_correct, false)
           AND EXISTS (SELECT 1 FROM questions q WHERE q.id = p_question AND q.qtype <> 'rarest') THEN
            parts := string_to_array(p_raw, ' — ');
            IF array_length(parts, 1) > 1 THEN
                SELECT qa.* INTO ans
                FROM generate_series(1, (1 << least(array_length(parts, 1), 4)) - 1) AS mask
                CROSS JOIN LATERAL (
                    SELECT array_to_string(ARRAY(
                        SELECT parts[i] FROM generate_series(1, array_length(parts, 1)) AS i
